...
$stmt = $db->prepare("SELECT * FROM " . $prefix['table_prefix'] . "_social_links WHERE id = :id");
...
<div class="col-md-12">
    <h4><?php echo t('Addon JD Flusity \'Social links\' ');?></h4>
    <div class="row d-flex">

    <form id="update-addon-form" method="POST" action="../../cover/addons/social_links/action/<?php echo $mode === 'edit' ? 'edit_addon_post.php' : 'add_addon.php'; ?>" enctype="multipart/form-data" class="col-md-10">
...
<!-- Other language end-->
        <div class="mb-3">
            <label for="simpleFormControlsocial_link_urlInput" class="form-label"><?php echo t('URL link');?></label>
            
            <input type="text" class="form-control" name="social_link_url" id="simpleFormControlsocial_link_urlInput" placeholder="url link" value="<?php echo $mode === 'edit' ? htmlspecialchars($addon['social_link_url']) : ''; ?>" required>
        </div>
        <button type="submit" name="submit" class="btn btn-primary"><?php echo t('Submit');?></button>
            <?php if (isset($_GET['addon_post_edit_id'])): ?>
            <a href="addons_model.php?name=social_links&id=<?php echo htmlspecialchars($_GET['id']) ?>" class="btn btn-secondary"><?php echo t('Cancel');?></a>
            <?php endif; ?>
        </div>
        <div class="col-md-4">
            <div class="mb-3">
            ...
            <button class="btn btn-primary" type="button" data-bs-toggle="offcanvas" data-bs-target="#offcanvasRight" aria-controls="offcanvasRight"><?php echo t('files Library');?></button>
                ...
                <div class="offcanvas offcanvas-start" tabindex="-1" id="demoOffcanvas" aria-labelledby="demoOffcanvasLabel">
                    ...
                    <div class="d-flex justify-content-end">
                        <img id="demosocialImage" class="d-block mt-5"  width="80" height="80" title="title name" alt="title name" src="/cover/addons/social_links/assets/img/facebook-1.png">
                    </div>
                    </div>
                </div>
                <!-- Demo view end -->
        ...
        <div class="col-md-12">
            <table class="table table-sm">
                ...
                if (!empty($get_params)) {
                    ...
                    $stmt = $db->prepare("SELECT COUNT(*) FROM " . $prefix['table_prefix'] . "_social_links");
                    $stmt->execute();
                    $total_addons = $stmt->fetchColumn();
                    $stmt = $db->prepare("SELECT addons.id, addons.title, addons.social_link_url, addons.img_url, menu.name as menu_name, places.name as place_name, addons.menu_id
                    FROM " . $prefix['table_prefix'] . "_social_links as addons LEFT JOIN " .
                    ...
                    foreach ($addonResults as $addonRes) {
                        ...
                        echo '<td>';
                        echo '<a href="addons_model.php?name=social_links&id=' . htmlspecialchars($_GET['id']) . '&addon_post_edit_id=' . htmlspecialchars($addonRes['id']) . '"><i class="fa fa-edit"></i></a> ';
                        echo '<a href="../../cover/addons/social_links/action/delete_addon_post.php?name=social_links&id=' . htmlspecialchars($_GET['id']) . '&addon_post_id=' . htmlspecialchars($addonRes['id']) . '"><i class="fa fa-trash"></i></a>';
                        echo '</td>';
                        echo '</tr>';
                    } ?></form>
                </tbody>
